...
- 成员列表分「在住 / 已退租」，每条带**头像**（已退租的置灰），标注谁设过 PIN、房间、联系方式
...
npm run test:e2e     # 注册 / 建房 / 记账 / 多房间切换 / 头像 / 公告权限 / 越权 / 改密码 / PIN（157 项）
...
├── e2e-phase2.mjs             账号体系 / 多房间 / 头像 / 公告权限（157 项）
...
>
> 曾经想放过一条口子：「作者已经不是本房间在住成员时，放开给所有人删」。**否决了**——`POST /members/:id/leave` 目前不校验「只能退自己」（见下一条「退租不区分退谁」），两者一叠加就成了完整绕过：任何成员都能先退租作者 → 删光他的公告 → 再恢复在住，作者校验等于没有。**两条各自「危害有限」的宽松规则会乘出一个真的洞**，这条比它本身值得记。
>
> 真要解，得先把「谁能退谁」定下来。那是产品决策，不是代码问题。
...
**退租不区分"退谁"。** `POST /api/members/:id/leave` 只校验那个成员属于本房间，**不校验 `id` 就是当前登录者自己**。界面上每个在住成员那一行都有「退租」按钮，看起来是**有意**让室友之间互相代标记的（比如替已经搬走的人标记）；但后端没做这个区分，所以实际效果是「任何人能退任何人」。

> 这属于**需要产品决策**的一条，不是明显的 bug。危害有限——退错了可以用「恢复在住」撤回，历史账目也不受影响（软删除）。但如果你想限制成"只能退自己 + 由某人统一管理"，得先定规则再改。
>
> 顺带一提：这一条现在是另一条限制的**前置条件**——公告那条「作者退租后无人能删」之所以不能简单放宽，正是因为「任何人能退任何人」会让放宽变成一条绕过路径。先修这里，那边才有解。
