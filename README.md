...
npm run test:e2e     # 注册 / 建房 / 记账 / 多房间切换 / 头像 / 越权 / 改密码 / PIN（133 项）
...
├── e2e-phase2.mjs             账号体系 / 多房间 / 头像端到端（133 项）
...
**公告的「只有作者能改」只在前端。** 界面上编辑 / 删除按钮只对作者渲染（`Announcements.tsx` 里 `a.authorId === session.member.id`），但服务端 `PATCH` / `DELETE /api/announcements/:id` **只校验房间归属，不校验 `author_id`**。也就是说任何房间成员直接调 API 就能改删别人的公告，前端那句注释描述的规则在后端并不存在。

> 这一条不是"设计如此"，是**前后端不一致**：UI 已经表达了意图，服务端没跟上。补服务端校验即可——但那会让「同一个房间里互相帮忙改个错别字」变得不可能，所以先记下来，改之前想清楚要不要允许。
