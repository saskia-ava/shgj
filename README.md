...
每个室友用**邮箱 + 密码**注册后，各自登录、看同一份共享数据。一个账号可以同时属于多个房间，每个房间一套独立的账本——顶栏的**＋ 房间**建新的，切回来用顶栏的下拉框。
...
- 建房自动生成 8 位邀请码
- 顶栏**＋ 房间**按钮：弹窗里二选一「新建房间 / 用邀请码加入」，提交完自动切到新房间
- 顶栏**房间切换下拉**——一个账号可以属于多个房间，各自独立账本；只有一个房间时下拉不出现
- 「已登录但当前房间没身份」（还没选房间，或已退租）有专门的中转页，三个出口：选已有房间 / 建新房 / 用邀请码加入
- 切房时整个界面强制重挂载（`key={householdId}`），防止把账记到错的房间

> **＋ 房间和下拉框是互补的，两个的显示条件刻意不同**：下拉框只在 `households.length > 1` 时出现（一个房间没什么好切的），而建第二个房间恰恰只能在只有一个房间的时候做。早先只有下拉框、没有这个按钮，于是「只有一个房间的账号」在界面上**没有任何入口**能再建或加入一个房间——只能退出登录重新注册。服务端一直支持（已登录时 `/auth/household` 和 `/auth/join` 都会把会话切到新房间，见 `auth.ts:541` / `auth.ts:671`），缺的纯粹是入口。
...
- 邀请码展示（点一下复制）
- 成员列表分「在住 / 已退租」，标注谁设过 PIN、房间、联系方式
...
代码里现在**完全不存在**的：图片上传、任何形式的推送、邮件发信、第三方登录、导出 / 报表 / 图表。`package.json` 的运行时依赖只有 3 个（hono、react、react-dom），这一点可以佐证。
...
npm run test:e2e     # 注册 / 建房 / 记账 / 多房间切换 / 越权 / 改密码 / PIN（112 项）
...
├── e2e-phase2.mjs             账号体系 / 多房间端到端（112 项）
