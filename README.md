...
给合租室友共用的账本与事务管理工具。**7 个页面**：总览、记账、结算、值日、公告、物品、室友。

每个室友用**邮箱 + 密码**注册后，各自登录、看同一份共享数据。一个账号可以同时属于多个房间，每个房间一套独立的账本。

**切换房间、邀请室友、修改密码、设置 PIN、更换头像、退出登录，全部收在右上角的「头像菜单」里**——点右上角的头像或名字展开，见[头像菜单](#头像菜单右上角)。
...
推送到 `main` 会自动测试并部署到这个地址，见「CI 自动部署」。

## 功能清单

后端 42 个接口，全部真实挂载（没有"写了忘了挂"的）。下面按页面分组。

### 账号与登录

| 能力 | 说明 |
|---|---|
| 邮箱 + 密码注册 | 注册后**一次性**展示 8 个恢复码（`XXXX-XXXX`），只显示这一次 |
| 邮箱密码登录 | 连错 5 次锁 15 分钟 |
| PIN 快捷登录 | 输邀请码 → 从房间里选自己的名字 → 输 PIN。给不想记密码的室友用 |
| 用邀请码加入 | 填邀请码 + 邮箱 + 密码 + 名字，注册和入伙一步完成。房间里已有同名占位档案时**自动认领它**，历史账目不会断 |
| 忘记密码 | 用恢复码重置，每个码只能用一次 |
| 登出 | 头像菜单底部 |

### 头像菜单（右上角）

点顶栏右上角的**头像或名字**展开，一个面板里放六件事：**更换头像 / 切换房间 / 邀请新室友 / 修改密码 / 设置 PIN / 退出登录**。

> 为什么收成一个菜单：这几件事原来散在**顶栏**（房间下拉、退出、＋房间）和**「室友」页底部**（邀请码、改密码、PIN）两处，找一个功能得先猜它在哪个区域。现在统一一个入口，`view` 状态在根列表和子面板之间切换，返回键统一回根。
>
> **不是删掉，是搬家**——原来的能力一个没少，只是换了地方。

**头像**是预设白名单（16 个 emoji，`src/shared/avatars.ts` 前后端共用同一份），**不是上传**：上传要开 R2，而 R2 绑卡才能激活（见[还没做的](#还没做的)）。没设头像时按**名字首字**兜底，底色由名字哈希决定，所以同一个人每次进来颜色都一样。

- 头像存的是 **id**（`fox`）不是 emoji（`🦊`）——存 id 才能用白名单校验，存 emoji 等于开一个任意文本字段。将来要换成真图片时加 `url:<key>` 前缀值即可，**不需要再迁移一次**
- 头像上**只能改自己的**（`PATCH /members/:id` 里 `id !== memberId` → 403）。名字和房间仍允许帮室友改（人还没进来先替他建好是正当场景），但替别人挑头像没有任何合理场景，只有「给室友挂个丑头像」这一种用法

### 房间（一个房间一个账本）

- 建房自动生成 8 位邀请码
- 头像菜单 → 切换房间 → 列出账号所在的**全部房间**，各自独立账本
- 同一面板底部**「＋ 新建 / 用邀请码加入房间」**：弹窗里二选一「新建房间 / 用邀请码加入」，提交完自动切到新房间
- 「已登录但当前房间没身份」（还没选房间，或已退租）有专门的中转页，三个出口：选已有房间 / 建新房 / 用邀请码加入
- 切房时整个界面强制重挂载（`key={householdId}`），防止把账记到错的房间

> **「切换房间」列表和它底部的「新建 / 加入」是互补的，两个的显示条件刻意不同**：房间列表里当前房间那颗按钮是禁用的（一个房间没什么好切的），而建第二个房间恰恰只能在房间少的时候做。早先界面上只有切换、没有入口，于是「只有一个房间的账号」**没有任何办法**能再建或加入一个房间——只能退出登录重新注册。服务端一直支持（已登录时 `/auth/household` 和 `/auth/join` 都会把会话切到新房间，见 `auth.ts:541` / `auth.ts:671`），缺的纯粹是入口。

> 建房 / 加入的表单只有一份实现（`HouseholdForms.tsx`），中转页和这个弹窗共用，免得两处各写一遍然后慢慢长歪。
>
> ⚠️ **菜单里「切换房间」和「新建 / 加入房间」是两件事，别合并**：前者是换一个已有房间的视图，后者是加一个新房间再切过去。合并之后「只有一个房间的人想建第二个房间」就又没入口了——这正是上面那段说的那个坑。

### 记账

- 分类：房租 / 水电 / 燃气 / 网费 / 日用 / 维修 / 其他，**分页 + 分类筛选**，显示本页合计
- 字段：花在什么上、金额、分类、日期、**谁垫的钱**、怎么分、谁参与、备注
- **均摊**：勾选参与人，实时预览每人分摊额，除不尽时提示余数规则
- **自定义分摊**：每人一行金额，实时显示「已分 X / 总额 Y（还差 Z）」，分不平不让提交
- 展开看「谁分摊了这 ¥X」的明细；可修改、可删除

### 结算

- 每人净额（应收 / 应付 / 已结清），已退租的人也在列表里带标记
- **最少笔数结算建议**：不是简单两两相抵，会算出「结清只需 N 笔转账」
- 每笔建议旁边一个「已转」按钮，一键记账
- 也能手动记一笔实际转账；**结算历史**可翻页、可撤销

### 值日

- 建项目：名称、周期（每天 / 每周 / 每月）、**按点击顺序**选轮转成员
- 显示当前轮到谁、本期完成没有；「标记完成」自动轮转到下一个人
- 可暂停 / 恢复 / 删除

### 公告

- 发布（标题 + 内容）、**置顶**、编辑、删除
- 置顶优先、时间倒序；总览页显示最近 3 条
- **权限分界是「内容」和「展示位置」**，前后端严格一致：

| 操作 | 谁可以 | 为什么 |
|---|---|---|
| 改标题 / 内容 | 只有作者 | 改的是别人写的话 |
| 删除 | 只有作者 | 不可逆，比编辑更重 |
| 置顶 / 取消置顶 | 房间内所有人 | 改的是「这条排多前」，不是内容 |

> ⚠️ 后端这个校验写在**字段级**（`if (('title' in body \|\| 'content' in body) && …)`），不是加在整个 `PATCH` 上。前端把**置顶按钮画给所有人**、编辑按钮只画给作者；整接口加一句作者校验会让那颗人人可见的置顶按钮开始吃 403，看起来像按钮坏了。E2E 第 14 节里「乙置顶别人的公告 200」就是专门钉这一条的——哪天有人把校验提到接口级，挂的会是它。
>
> 这段校验是后补的：原来前端只画给作者、后端只看房间归属，是一条**前后端不一致**（详见「已知限制」里那条「作者退租后」）。

### 公共物品

- 添加物品：名称、单位、现有数量、**低于多少提醒补货**
- **±1 按钮**做原子增减（服务端 `MAX(0, quantity + ?)`，防并发丢更新）
- 低于警戒线的排前面；三个地方提醒：侧边角标、总览页区块、物品页横幅
- 可编辑、可删除

### 账号安全

> 入口在**头像菜单 → 修改密码**，不是独立 tab。

- **改密码（两段式）**：先验当前密码，再设新密码；成功后**踢掉其它设备上的登录**
- **重设恢复码**：输当前密码 → 旧的立即全部作废 → 新的一批展示一次

### 室友

- 成员列表分「在住 / 已退租」，每条带**头像**（已退租的置灰），标注谁设过 PIN、房间、联系方式；没被账号认领的档案标一个「未认领」标签
- 添加室友（占位，不发 PIN——让对方自己注册后认领）
- 标记退租（**软删除**，历史账目保留原名字）、恢复在住
- 👉 退租 / 恢复**只能操作自己的身份**，占位档案（还没有账号的）例外——见下面的权限表
- 设置 / 修改我在这个房间的 PIN → 头像菜单 → 设置 PIN
- 邀请码展示（点一下复制）→ 头像菜单 → 邀请新室友

**退租 / 恢复的权限**

| 目标档案 | 谁能退租 / 恢复 | 为什么 |
|---|---|---|
| **已被账号认领** | 只有本人 | 这两个端点的权限是一样的。`restore` 会把 `is_active` 翻回 1，而中间件正是按 `is_active = 1` join 出成员身份的——**恢复一个人等于把他的账号重新放进这个房间**，能看全部账目。那不是「标记错了可以撤回」，是单向的授权操作 |
| **占位档案**（`account_id IS NULL`） | 房间里的任何人 | 它没有账号，**永远没法退自己**。一律锁成「只能退自己」的话，一个最终没搬进来的占位档案会永久卡在「在住」名单里——没有删除端点，只有退租 |

> ⚠️ 这条规则有个**设计后果**，不是 bug：已认领的成员**自己**退租之后，`restore` 对他实际不可达——别人替他调是 403，他自己调时已经没有成员身份、只会拿到 `NO_MEMBERSHIP`。唯一的回头路是**用邀请码重新加入**，`/auth/join` 会沿用原来的 `members.id`（不是新建身份），历史账目一条不丢。所以 `HouseholdGate` 上专门写了这句提示。
>
> 前端画按钮用的是 `GET /members` 返回的 `hasAccount` 字段，和服务端 `canManage()` 是同一条规则——**只是不让按钮白出现，不是权限**。

### 还没做的

| 期 | 内容 | 卡在哪 |
|---|---|---|
| 三 | 图片上传（发票照片） | 需要开通 R2 并绑信用卡 / PayPal（额度内不扣费） |
| 四 | 消息推送（PushPlus 推微信 + 浏览器通知） | PushPlus 需要实名认证 |
| 五 | 邮箱验证 + 邮件找回密码 + 邮件通知 | 需要买域名 |

**手机号只作为不验证的联系方式存着**，不做短信验证——短信需要签名报备，个人账号的「自用资质」过不了运营商报备，需要营业执照。

代码里现在**完全不存在**的：**图片上传**（头像是 16 个预设 emoji 的白名单，不碰存储）、任何形式的推送、邮件发信、第三方登录、导出 / 报表 / 图表。`package.json` 的运行时依赖只有 3 个（hono、react、react-dom），这一点可以佐证。
...
npm run test:e2e     # 注册 / 建房 / 记账 / 多房间切换 / 头像 / 公告权限 / 退租权限 / 越权 / 改密码 / PIN（192 项）
...
**这个 LEFT JOIN 本身就是授权检查，不只是查询优化。** 把 `active_household_id` 手工改成别人家的房间 id，join 不出成员行 → 403，而不是拿着一个不属于自己的身份去查数据。`npm run test:e2e` 的第 13 节就是直接改库来验证这条防线（fail closed）。
...
- 每个码只能用一次
- 弄丢了可以在**头像菜单 → 修改密码 → 重设恢复码**用密码换一批新的，**旧的立即全部作废**
...
1. **`0001_init.sql` 之后永不修改。** 它是纯 CREATE，不含任何 DROP，是整条迁移链的基线。
   ...
   **注意它的内容是「第二期之后」的状态**：`accounts` / `member_pins` / `recovery_codes`、`sessions` 的 `account_id` + `active_household_id`、`idx_members_account_active` 部分唯一索引都已经在里面了。

   这是刻意的。第二期走的是「线上清库重建」而不是写数据迁移（理由见下），所以基线被**一次性重写**成重建后的形状，然后冻结——**`0002` 才是第一个增量迁移**（`members.avatar`，见下）。
...
| 文件 | 内容 |
|---|---|
| `0001_init.sql` | 基线，13 张表 + 9 个索引。**永不修改** |
| `0002_member_avatar.sql` | `ALTER TABLE members ADD COLUMN avatar TEXT;` |

`0002` 这条值得说明三点：

- **NULL 是合法且有意义的默认值**，表示「没设过头像，用名字首字兜底」。所以**不需要回填数据**，也**不能**给它 `NOT NULL` + `DEFAULT ''`——那样「没设过」和「设了个空字符串」就分不出来了
- **放在 `members` 而不是 `accounts`**：头像是「你在**这个房间里**长什么样」，和名字同源。放账号上会得到「在 A 房叫小明、B 房叫明哥，头像却是同一张」这种撞车，和「昵称不上账号」是同一个理由
- `ALTER TABLE ADD COLUMN` 是 SQLite 少数几个**不需要重建表**的 schema 变更之一，所以它不会碰 `members.id`，正合上面那条纪律
...
```
migrations/          D1 迁移（唯一改 schema 的地方）
├── 0001_init.sql      基线，永不修改
└── 0002_member_avatar.sql
scripts/
├── verify-integrity.sql       完整性检查语句
├── verify-integrity.mjs       跑上面的 SQL 并断言全为 0
├── e2e-phase2.mjs             账号体系 / 多房间 / 头像 / 公告权限 / 退租权限（192 项）
├── measure-cpu.mjs            线上 cpuTime 实测（配合 wrangler tail）
└── reset-remote-db.sql        ⚠️ 清空线上库，**绝不能**放进 migrations/
src/
...
│   └── avatars.ts     头像白名单（worker 校验和前端九宫格读同一份）
├── worker/          后端
...
└── client/          前端
    ...
    ├── App.tsx        三态路由、顶栏、头像菜单挂载、＋ 房间弹窗
    ├── components/    可复用的交互件
    │   ├── Avatar.tsx          预设 emoji / 名字首字兜底
    │   ├── AvatarMenu.tsx      ★ 顶栏那个六合一面板
    │   ├── AccountSecurity.tsx 改密码 + 重设恢复码（内嵌进菜单）
    │   ├── InviteBox.tsx       邀请码展示与复制
    │   ├── PinForm.tsx         设置 / 修改本房间 PIN
    │   └── useDismissable.ts   Esc / 点外关闭 / 焦点归还，弹层共用
    └── pages/         各页面；AuthPage 未登录、HouseholdGate 未选房间
        └── HouseholdForms.tsx   建房 / 加入表单，中转页和弹窗共用这一份
test/                单测
```

`useDismissable` 里有一处**看着多余、删了会坏**的写法：`onClose` 存进 `closeRef` 而不是进依赖数组。它是每次渲染都换身份的函数，进依赖数组会让 effect 每渲染重跑一次，而 effect 开头是「把焦点移到第一个可聚焦元素」——结果是**每按一个键焦点都被拽回第一个输入框**，中文输入法直接没法用。症状离原因很远，所以留了注释。
...
- **没有测试覆盖的端点会坏得无声无息。** `GET /members` 的 SELECT 里写着裸的 `pin_hash`，而第二期已经把这一列挪到了 `member_pins`——于是这个端点每次请求都 500（`no such column: pin_hash`），从 `9699a7d` 一直活到 2026-09-14 才被发现。

  之所以没人发现：**E2E 全程用 `/auth/me`、`/balance`、`/expenses`，从没调过它**；而前端 `reloadMembers()` 是个被 `void` 掉的 promise，500 只会让成员列表静默变空、名字显示成「未知成员」，不白屏也不报错。所以它表现得像「有点怪」而不是「坏了」。
...
- **断言 403 时，要连 error 文案一起断言，光看状态码会被「因为别的理由失败」骗过去。**

  写第 15 节（退租权限）时实测到的：把 `canManage` 短路成 `return true` 之后，「甲退乙 403」「甲恢复乙 403」**依然 PASS**——因为「乙退甲 403」真的变成了 200，甲当场失去成员身份，于是甲后面任何请求都返回 403，只不过理由是 `NO_MEMBERSHIP` 而不是 `只能退租自己的身份`。

  也就是说，**两条最核心的断言是因为错误的理由通过的**。改成同时比 `body.error` 之后，短路下失败项从 10 条涨到 14 条，那 4 条正是它们。

  通用的判断法：一条断言如果**在「把被验证的代码删掉」之后仍然通过**，它就没在验证那件事。这类越权断言尤其容易中招，因为「没有权限」和「没有身份」恰好都是 403。
...
**公告：作者退租之后，他发的公告没人能改也没人能删。** 改内容和删除现在**只有作者本人**能做（后端真的校验了 `author_id`，见[公告](#公告)那节的权限表）。而成员是软删除的——人走了，`members` 行还在，那个身份却再也登不进来。于是他的公告**永久冻结**：置顶还能动，内容改不了、也删不掉。

> 这是「严格作者制」的**直接代价，是自觉选的**，不是漏掉的分支。
>
> 曾经想放过一条口子：「作者已经不是本房间在住成员时，放开给所有人删」。**当时否决的理由是**：`POST /members/:id/leave` 不校验「只能退自己」，两者一叠加就成了完整绕过——任何成员都能先退租作者 → 删光他的公告 → 再恢复在住。**两条各自「危害有限」的宽松规则会乘出一个真的洞**，这条比它本身值得记。退租权限已经收紧了（见下一条），那条具体的绕过路径没有了，但**结论不变**：放宽删除权仍然需要一个「谁算管理员」的概念，现在没有，所以任何成员都能删光前室友的公告。那是产品决策，不是代码问题。

**（已修）公告的「只有作者能改」曾经只在前端。** 界面上编辑 / 删除按钮只对作者渲染，服务端却只校验房间归属——任何房间成员直接调 API 就能改删别人的公告。这是一条典型的前后端不一致：UI 已经表达了意图，服务端没跟上。

> 修的时候踩到一个坑，值得单独记：**修法不能是「给 `PATCH` 加一句作者校验」，因为那会把「置顶」一起锁掉**——置顶按钮对所有人渲染，非作者一点就 403。分界必须按字段切（内容 / 展示位置），见[公告](#公告)。E2E 第 14 节为此专门留了一条「乙置顶别人的公告 200」。

**（已修）退租不区分"退谁"。** 原来 `POST /api/members/:id/leave` 和 `/restore` 只校验那个成员属于本房间，**不校验 `id` 就是当前登录者自己**——实际效果是「任何人能退任何人、也能恢复任何人」。现在按「**只能操作自己的身份 + 占位档案例外**」收紧了，规则和理由见[室友](#室友)那节的权限表。

> 收的是**两头**，因为 `restore` 才是重的那一头：它把 `is_active` 翻回 1，等于把那个账号重新放进房间。只锁 `leave` 的话，「先退掉室友、再把他恢复成别人的身份」这条路仍然通着。
>
> 留下一个后果是有意的，已写进上面那张表：**已认领成员自己退租后，`restore` 对他不可达**，只能靠邀请码重新加入接回原档案。想加「自己恢复自己」的话得让 `/restore` 在 `NO_MEMBERSHIP` 下也可达——那需要一个新的授权依据（比如会话里记「你刚从这个房间退租」），不是把检查放松回去。
>
> 兑现了上面公告那条的**前置条件**：作者退租后无人能删的问题，现在至少不再是「被退租绕过」的了。但那条本身仍然**未修**——放宽删除权限要先有「谁算管理员」这个概念。

**破坏性操作都没有二次确认。** 退租、删账目、删公告、删物品、撤销结算，点了就生效。`POST /api/members/:id/leave` 可以「恢复在住」撤回，其余的删了就没了。前端加确认框即可，属于体验问题不是安全问题。

**`emailVerified` 是个空字段。** 三处注册（`register` / `join` / 匿名建房）都写死 `email_verified = 0`，**没有任何代码路径能把它变成 1**，前端也不读它。这是给第五期（邮件验证）预留的占位，目前只有列和字段、没有功能。

**`PIN_PEPPER` 是唯一的线上硬依赖，而 CI 不校验它。** 它一旦丢失，所有人既登不上也无法运维重置（密码和 PIN 全部依赖它派生）。CI 的部署 job 只检查 `CLOUDFLARE_API_TOKEN`，不碰这个 secret——它是人工用 `wrangler secret put` 提前写好的。备份在 `.pin-pepper.backup.txt`（gitignored），**本地副本、线上 secret、备份文件这三处至少要有两处同时存在**。
